...
<br></br>
## 절차 지향 프로그래밍
* 순차적으로 진행(Top-Down 방식)하면서 '프로그램은 무슨 일을 하는가'의 관점으로 바라본 프로그래밍 기법
* 기능을 하나의 함수로 구현 (**기능 중심**)
* e.g.) C언어
<br></br>
...
#### 특징:
* **데이터 중심**
* **Bottom-up** 방식으로 작은 단위의 문제를 해결할 수 있는 객체를 만든 뒤, 여러 작은 단위의 객체들을 이용하여 큰 문제를 해결하는 것을 반복하는 컴퓨터 패러다임(복잡한 logic에 사용이 용이함)
...
2. **추상화**: 구체적인 정보가 아닌 추상적인 정보를 제공하여 구체적인 정보를 위한 객체를 만들기 위해 추상화 설계가 필요
#### 3요소:
...
<br></br>
#### 기초 5원칙 (SOLID)
* **S**RP(Single responsibility principle, 단일 책임 원칙): 단 한개의 기능을 가져야함
* **O**CP(Open-closed principle, 개방 폐쇄 원칙): 소프트웨어 개체(클래스, 모듈, 함수 등)은 확장에 Open되어있어야 하고, 수정에는 Close 되어있어야함. (기능 확장은 OK, 기존 코드의 수정은 NOPE!)
